...
/* ── Random-length hex name (8, 12, or 16 hex chars) ── */
static int rand_hex_len(void) {
    unsigned char buf;
    RAND_bytes(&buf, 1);
    int r = buf % 3;
    return (r == 0) ? 8 : (r == 1) ? 12 : 16;
}

/* ── HMAC-SHA256 context-aware name mangling with variable length ── */
char *rename_mangle_name(const char *original, int scope_depth,
    ...
    int hex_chars = rand_hex_len();

    /* Vary prefix: _, __, or _0 */
    unsigned char prefix_buf;
    RAND_bytes(&prefix_buf, 1);
    int pfx = prefix_buf % 3;
    int off = (pfx == 0) ? 1 : 2;

    size_t buf_size = (size_t)(off + hex_chars * 2 + 1);
    ...
    if (pfx == 0) {
        ...
        result[0] = '_'; result[1] = '_';
    } else {
        result[0] = '_'; result[1] = '0' + (prefix_buf % 9);
    }

    for (int i = 0; i < hex_chars; i++)
        snprintf(result + off + i * 2, 3, "%02x", digest[i % 16]);
    result[off + hex_chars * 2] = '\0';
    return result;
}
...
int rename_generate_decoys(RenameTable *tbl, int count,
    ...
    static const char *decoy_names[] = {
        ...
        "placeholder", "accumulator", "iterator", "mapping", "filter",
        nullptr
    };
    static const char *dunder_like[] = {
        "__secret__", "__magic__", "__hidden__", "__cached__",
        nullptr
    };
    int generated = 0;
    for (int i = 0; i < count; i++) {
        ...
        unsigned char sel;
        RAND_bytes(&sel, 1);
        if (sel % 4 == 0 && dunder_like[i % 3]) {
            name = dunder_like[i % 3];
        } else {
            name = decoy_names[i % 20];
        }
        int depth = (i % 3) + 1;
        char parent[32];
...
char *rename_generate_python(const char *input_source,
    ...
    /* Also rename dunder methods (__init__ → _, __str__ → __, etc.) */
    ...
    py += "    def visit_ClassDef(self, node):\n";
    py += "        if node.name in self.name_map:\n";
    py += "            node.name = self.name_map[node.name]\n";
    py += "        self.generic_visit(node)\n";
    py += "        return node\n";
    ...
    /* Use flat dict (original name → mangled name), no scope tuples */
    py += "_MAP = {\n";
    for (size_t i = 0; i < tbl->count; i++) {
        std::string esc_orig;
