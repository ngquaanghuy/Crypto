...
static void init_tbl(void) {
    ...
    size_t len = strlen(TBL_ENCODE);
    for (size_t i = 0; i < len; i++)
        TBL_DECODE[(unsigned char)TBL_ENCODE[i]] = (signed char)i;
    done = 1;
}
...
ExitCode base85_encode(const unsigned char *in, size_t in_size, Buffer *out) {
    ...
    size_t out_size = full * 5 + (rem > 0 ? rem + 1 : 0) + 1;
...
ExitCode base85_decode(const unsigned char *in, size_t in_size, Buffer *out) {
    ...
    while (i < in_size) {
        ...
        unsigned long n = 0;
        int nchars = 0;
        while (nchars < 5 && i < in_size && !strchr(" \t\n\r", in[i])) {
