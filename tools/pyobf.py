...
class _StateEncoder(ast.NodeTransformer):
    ...
    def visit_While(self, node):
        ...
        # 1. Encode initial state assignment (first stmt before while)
        #    We need to find the parent node to modify the initial assignment.
        #    Instead, walk the while node and encode all refs inside.
        self._encode_state_refs(node)

        # 2. Encode the initial assignment *before* the while loop.
        #    We do this by looking at the entire function body in the caller.
        #    The initial assignment is:
        #        __fs = first_state_val  →  __fs = _F(first_state_val)
        #    This is handled by the FunctionDef visitor below.

        # 3. Inject encoding function definition before while
        #    We'll prepend it in the FunctionDef visitor.
        return node
